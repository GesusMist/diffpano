"""Aligned 512 RGB-pixel patch / 32 RGB-pixel stride follow-up to I/J."""
import copy
import hashlib
import json
...
ROOT = Path('outputs/planar-dense-ij/20260918-p512-s32')
...
def sources():
    paths = [p for folder in ('diffpano', 'scripts', 'studies/planar_dense_ij')
...
def spec_config(backend, method):
    ...
    config.native_multidiffusion.patch_size = 512//factor
    config.native_multidiffusion.stride = 32//factor
    config.validate()
...
def make_manifest():
    ...
    for backend, factor in FACTORS.items():
        for method in ('I', 'J'):
            ...
            rgb = build_planar_patch_layout(n.canvas_height*factor, n.canvas_width*factor, 512, 32)
            assert native.num_patches == rgb.num_patches == (17 if backend == 'sd2' else 833)
    ...
    for b in FACTORS:
        ...
        assert ac==jc
    return dict(study='I/J planar dense overlap', rgb_patch_size=512, rgb_stride=32,
        schedule_policy='Existing backend prepare at actual local 512x512 resolution; identical within each I/J pair. FLUX dynamic shift is resolution-derived.',
