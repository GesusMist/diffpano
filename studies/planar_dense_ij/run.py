...
from studies.planar_dense_ij.common import FACTORS, read, write, validated_row
...
def check_runtime(runtime, old, backend):
    ...
    assert runtime['native_geometry']['local_rgb_shape']==[512,512]
...
def run(name, method):
    ...
    with torch.no_grad():
        ...
        assert tuple(rgb.shape[-2:])==(512,512) and tuple(encoded.shape)==shape
