#!/usr/bin/env python3
"""Experiment I: change G's post-fusion transition only; reuse every saved control setting."""
import argparse
import copy
import json
import os
...
def check_configs(spec):
    ...
    expected['consensus_transition'] = {'mode': 'preserve_current_state'}
    if config.to_dict() != expected:
        raise AssertionError('I must differ from G only in consensus_transition.mode')
...
def check_runtime(current, baseline):
    ...
    for key in keys:
        if current.get(key) != baseline.get(key):
            raise AssertionError('I/G runtime mismatch: '+key)
    for key in ('python', 'torch', 'cuda'):
        if current['environment'][key] != baseline['environment'][key]:
            raise AssertionError('I/G software mismatch: '+key)
    return keys
...
def run(spec):
    ...
    if torch.cuda.get_device_name() != spec['gpu']:
        raise AssertionError('I requires the saved A40 control GPU type')
    ...
    if conditioning_digest(conditioning) != expected_condition:
        raise AssertionError('I/G conditioning hash mismatch')
    ...
    if initial_hash != reference['initial_native_sha256']:
        raise AssertionError('I/G saved initial tensor hash mismatch')
    ...
    if actual_count != expected_count or result.audit['guided_predictions'] != expected_count:
        raise AssertionError('I/G guided prediction count mismatch')
    if conditioning_digest(conditioning) != expected_condition:
    ...
    result.audit['transition_diagnostics_source'] = 'counterfactual G and actual I on identical I trajectory states'
    ...
    write(folder/'comparison.json', dict(experiment='I', backend=spec['backend'], config=config.to_dict(),
    ...
    print('Completed I', spec['backend'], folder, flush=True)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--spec', default='configs/experiments/vae_residual/i-all-models.json')
