#!/usr/bin/env python3
"""Experiment J: change F's post-fusion transition only; reuse every saved control setting."""
import argparse
import copy
import json
import os
...
def check_configs(spec):
    ...
    expected['consensus_transition'] = {'mode': 'preserve_current_state', 'vae_residual_correction': False}
    if config.to_dict() != expected:
        raise AssertionError('J must differ from F only in consensus_transition.mode')
...
def check_runtime(current, baseline):
    ...
    for key in keys:
        if current.get(key) != baseline.get(key):
            raise AssertionError('J/F runtime mismatch: '+key)
    for key in ('python', 'torch', 'cuda'):
        if current['environment'][key] != baseline['environment'][key]:
            raise AssertionError('J/F software mismatch: '+key)
    return keys
...
def run(spec):
    ...
    if torch.cuda.get_device_name() != spec['gpu']:
        raise AssertionError('J requires the saved A40 control GPU type')
    ...
    if conditioning_digest(conditioning) != expected_condition:
        raise AssertionError('J/F conditioning hash mismatch')
    ...
    if initial_hash != reference['initial_native_sha256']:
        raise AssertionError('J/F saved initial tensor hash mismatch')
    pixel_native = spec['backend'] == 'pixeldit'
    pipe = PlanarImpliedEndpointConsensusPipeline(native_config=config.native_multidiffusion,
        backend=backend, pixel_native=pixel_native, residual_correction=False, roundtrip_diagnostics=False,
        fusion_config=config.fusion, transition_mode=config.consensus_transition.mode,
        flow_transition=spec['backend'] != 'sd2')
    ...
    if actual_count != expected_count or result.audit['guided_predictions'] != expected_count:
        raise AssertionError('J/F guided prediction count mismatch')
    if conditioning_digest(conditioning) != expected_condition:
        raise AssertionError('Conditioning mutated')
    result.audit['initial_local_states_equal_global_native_crops'] = True
    result.audit['vae_residual_correction'] = False
    result.audit['own_rgb_roundtrip_diagnostic'] = False
    result.audit['consensus_transition'] = config.consensus_transition.mode
    result.audit['transition_diagnostics_source'] = 'counterfactual F and actual J on identical I trajectory states'
    write(folder/'transition_patches.json', result.transition_diagnostics)
    import csv
    with (folder/'transition_patches.csv').open('w', newline='') as stream:
        writer = csv.DictWriter(stream, fieldnames=list(result.transition_diagnostics[0]))
        writer.writeheader(); writer.writerows(result.transition_diagnostics)
    save_result(folder/'generation', config, backend, result, timing)
    check_runtime(read(folder/'generation/metadata.json'), baseline)
    write(folder/'comparison.json', dict(experiment='J', backend=spec['backend'], config=config.to_dict(),
        reference=spec['reference'], reference_generation=spec['reference_generation'], baseline_label=spec['baseline_label'],
        only_setting_changed='consensus_transition.mode: preserve_prefusion_endpoint -> preserve_current_state',
    ...
    print('Completed J', spec['backend'], folder, flush=True)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--spec', default='configs/experiments/implied_endpoint_consensus/j-all-models.json')
    parser.add_argument('--backend', required=True, choices=['sd2', 'sana', 'flux', 'sd35'])
