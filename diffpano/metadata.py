...
def build_run_metadata(config: Any, pipe: Any, call_kwargs: Mapping[str, Any], output_file: str) -> Dict[str, Any]:
    """Build canonical run metadata while keeping the resolved config in its own YAML file."""

    class ConfigArgs:
        pretrained_model_name_or_path = config.model.path or config.model.id
        revision = config.model.revision
        variant = config.model.variant
        mixed_precision = config.model.precision
        prompt_to_log = None

    metadata = build_output_metadata(ConfigArgs(), pipe, call_kwargs, output_file)
    metadata["schema_version"] = 3
    metadata["experiment"] = {"name": config.experiment.name, "seed": config.experiment.seed}
    metadata["prompts"] = _json_safe(getattr(config, "resolved_prompts", None))
...
def save_run_metadata(
    ...
    metadata = build_run_metadata(config, pipe, call_kwargs, output_file)
